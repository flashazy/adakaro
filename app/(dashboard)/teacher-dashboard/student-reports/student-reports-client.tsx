...
export function StudentReportsClient({
  students,
}: {
  students: StudentMarksSummaryRow[];
}) {
  ...
  const filtered = useMemo(() => {
    ...
    return students.filter((s) => {
      ...
      const grade = (s.approximateGrade ?? "").toLowerCase();
      return (
        name.includes(q) ||
        adm.includes(q) ||
  ...
  return (
    <div>
      <div className="mb-4 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div className="relative min-w-0 flex-1 sm:max-w-xl">
          <input
            ...
            placeholder="Search by name, admission #, class, or grade..."
        ...
        <label className="flex w-full items-center gap-2 sm:w-auto">
          ...
          <select
            ...
            onChange={(e) => {
              ...
              setRowsPerPage(n);
              setCurrentPage(1);
              localStorage.setItem(TEACHER_STUDENTS_ROWS_STORAGE_KEY, String(n));
            }}
      ...
      {filtered.length > 0 ? (
            ...
            <div className="max-h-[min(70vh,720px)] overflow-x-auto overflow-y-auto">
              <table className="w-full min-w-[720px] table-fixed border-collapse">
                <thead className="sticky top-0 z-10 border-b border-slate-200 bg-white dark:border-zinc-800 dark:bg-zinc-900 [&_th]:bg-white dark:[&_th]:bg-zinc-900">
                  <tr>
                    <th className="w-[100px] px-4 py-3 text-left text-sm font-medium text-gray-500 dark:text-zinc-400">
                      ADM #
                    </th>
                    <th className="min-w-[180px] px-4 py-3 text-left text-sm font-medium text-gray-500 dark:text-zinc-400">
                      Student Name
                    </th>
                    <th className="w-[120px] px-4 py-3 text-left text-sm font-medium text-gray-500 dark:text-zinc-400">
                      Class
                    </th>
                    <th className="w-[64px] px-4 py-3 text-left text-sm font-medium text-gray-500 dark:text-zinc-400">
                      Gender
                    </th>
                    <th className="w-[88px] px-4 py-3 text-left text-sm font-medium text-gray-500 dark:text-zinc-400">
                      Marks avg
                    </th>
                    ...
                    <th className="sticky right-0 z-30 w-[120px] min-w-[120px] border-l border-slate-200 bg-white px-2 py-3 text-left text-sm font-medium text-gray-500 shadow-[-6px_0_8px_-6px_rgba(15,23,42,0.12)] dark:border-zinc-800 dark:bg-zinc-900 dark:text-zinc-400 dark:shadow-[-6px_0_8px_-6px_rgba(0,0,0,0.35)]">
                      Actions
                    </th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-slate-200 dark:divide-zinc-800">
                  {pageSlice.map((s) => (
                    <tr key={s.id}>
                      ...
                      <td className="px-4 py-3 text-sm tabular-nums text-slate-800 dark:text-zinc-200">
                        {pct(s.marksAveragePercent)}
                      </td>
                      <td className="px-4 py-3 text-sm font-medium text-slate-800 dark:text-zinc-200">
                        {s.approximateGrade ?? "—"}
                      </td>
                      <td className="px-4 py-3 text-sm tabular-nums text-slate-600 dark:text-zinc-400">
                        {s.marksCount}
                      </td>
                      <td className="sticky right-0 z-20 border-l border-slate-200 bg-white px-2 py-3 shadow-[-6px_0_8px_-6px_rgba(15,23,42,0.12)] dark:border-zinc-800 dark:bg-zinc-900 dark:shadow-[-6px_0_8px_-6px_rgba(0,0,0,0.35)]">
                        <Link
                          ...
                          className="inline-flex rounded-md border border-[rgb(var(--school-primary-rgb)/0.25)] px-2.5 py-1.5 text-xs font-medium text-school-primary hover:bg-[rgb(var(--school-primary-rgb)/0.10)] dark:border-[rgb(var(--school-primary-rgb)/0.32)] dark:text-school-primary dark:hover:bg-[rgb(var(--school-primary-rgb)/0.18)]"
                        >
                          View Profile
                        </Link>
                      </td>
                ...
                {pageNumbers.map((item, idx) =>
                  item === "ellipsis" ? (
                    ...
                    <button
                      ...
                      className={
                        item === currentPage
                          ? "min-w-[2rem] rounded-md bg-school-primary px-2 py-1 text-sm font-medium text-white dark:bg-school-primary"
                          : "min-w-[2rem] rounded-md px-2 py-1 text-sm text-slate-700 hover:bg-slate-100 dark:text-zinc-200 dark:hover:bg-zinc-800"
                      }
                    >
                      {item}
                    </button>
        ...
        <p className="mt-6 text-sm text-slate-600 dark:text-zinc-400">
