...
import {
  loadHistoricalMarksForClassTeacher,
  listStudentIdsWithHistoricalMarks,
  type HistoricalMarksClassGroup,
} from "@/lib/gradebook/load-historical-marks-for-class-teacher";

async function assertClassTeacherHistoricalMarksAccess(
  userId: string,
  classId: string
): Promise<
...
export async function listStudentsWithHistoricalMarksAction(input: {
  classId: string;
  studentIds: string[];
}): Promise<
...
> {
  ...
  const access = await assertClassTeacherHistoricalMarksAccess(user.id, classId);
  if (!access.ok) {
  ...
  const withHistory = await listStudentIdsWithHistoricalMarks(
    classId,
    access.schoolId,
    ids
  );
...
export async function loadStudentHistoricalMarksAction(input: {
  classId: string;
  studentId: string;
}): Promise<
  | { ok: true; groups: HistoricalMarksClassGroup[] }
  | { ok: false; error: string }
> {
  const classId = input.classId?.trim();
  ...
  const loaded = await loadHistoricalMarksForClassTeacher(
    studentId,
    classId,
    access.schoolId
  );
