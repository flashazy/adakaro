...
import {
  DASHBOARD_FEE_TYPES_ROWS_STORAGE_KEY,
...
export function FeeTypesList({ feeTypes }: { feeTypes: FeeTypeListItem[] }) {
  ...
  useEffect(() => {
    const stored = parseStudentListRowsPerPage(
      localStorage.getItem(DASHBOARD_FEE_TYPES_ROWS_STORAGE_KEY)
    );
  ...
  return (
    <div className="mt-6 space-y-2">
      <div className="rounded-xl border border-slate-200 bg-white p-3.5 shadow-sm dark:border-zinc-800 dark:bg-zinc-900">
        <div className="relative max-w-md">
          ...
          <input
            ...
            aria-label="Search fee types by name"
      ...
      <div className="flex flex-wrap items-center justify-between gap-2 border-b border-slate-200 py-2 dark:border-zinc-700">
      ...
      {totalFiltered === 0 ? null : (
        <>
          <div className="flex flex-col gap-2.5 sm:overflow-hidden sm:rounded-xl sm:border sm:border-slate-200 sm:bg-white sm:shadow-sm dark:sm:border-zinc-800 dark:sm:bg-zinc-900">
            ...
            <div className="flex flex-col gap-2.5 sm:divide-y sm:gap-0 sm:divide-slate-200 dark:sm:divide-zinc-800">
              {paged.map((ft) => (
          ...
          {totalPages > 1 ? (
            <nav
              ...
              aria-label="Fee types pagination"
            >
              <button
                type="button"
