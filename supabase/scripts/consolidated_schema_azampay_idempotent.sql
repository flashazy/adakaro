-- =============================================================================
-- Adakaro — Consolidated idempotent schema (AzamPay era, pre–ClickPesa)
-- =============================================================================
-- Source: supabase/migrations/00001 … 00019 (no ClickPesa migration in repo).
-- Run in Supabase SQL Editor on an empty or partial database.
--
-- Includes:
--   • Core tables: profiles, schools, school_members, classes, students,
--     fee_types, fee_structures, payments, receipts, parent_students,
--     parent_link_requests, azampay_pending_payments
--   • students.admission_number + optional contact fields used by the app
--   • payments.payment_method / reference_number (app + types; old cols renamed)
--   • View student_fee_balances, RPCs create_founding_school, get_my_school_id,
--     lookup_student_by_admission, RLS policies through 00019
--
-- Idempotent patterns: IF NOT EXISTS, DROP IF EXISTS, ADD COLUMN IF NOT EXISTS,
-- DO blocks for enums and conditional renames.
-- =============================================================================
...
ALTER TYPE public.payment_method ADD VALUE IF NOT EXISTS 'azampay';
...
-- ---------- NEW SCHOOL → FOUNDING ADMIN MEMBER ----------
...
AS $$
BEGIN
  -- Avoid duplicate row when RPC also inserts membership (ON CONFLICT is idempotent).
...
-- ---------- RPC: founding school (00017 JWT-aware) ----------
...
-- azampay_pending_payments
DROP POLICY IF EXISTS "Parents can insert own pending" ON public.azampay_pending_payments;
...
-- =============================================================================
-- Done. Optional: refresh PostgREST schema cache in Dashboard → Settings → API.
-- =============================================================================
