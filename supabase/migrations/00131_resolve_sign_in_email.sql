-- Mobile + clients: resolve non-email login identifiers to the Supabase Auth email,
-- mirroring web server logic in lib/resolve-teacher-login-email.ts (order: capture
-- card username → parent admission → teacher/admin full name). Callable pre-session
-- by anon so Flutter can sign in with signInWithPassword after resolution.
...
BEGIN
  ...
  SELECT coalesce(
      array_agg(DISTINCT trim(p.email)) FILTER (WHERE trim(coalesce(p.email, '')) <> ''),
      ARRAY[]::text[]
    )
