...
import {
  HistoricalClassRecordsEmptyState,
  HistoricalClassRecordsGroupedList,
} from "@/components/class-teacher/historical-class-records-display";
import type { HistoricalClassListClassGroup } from "@/lib/teacher-attendance/load-historical-class-list-for-class-teacher";

export function ViewPreviousClassRecordsLink({
  onClick,
}: {
  onClick: () => void;
}) {
  return (
    ...
    >
      View previous class records
    </button>
  );
}

export function PreviousClassRecordsModal({
  open,
  studentName,
  groups,
  loading,
  ...
  return createPortal(
    ...
    >
      ...
      <div
        role="dialog"
        aria-modal="true"
        aria-labelledby="previous-class-records-title"
        aria-describedby="previous-class-records-desc"
      ...
      >
        <div className="relative shrink-0 border-b border-slate-200 px-4 py-4 dark:border-zinc-700 sm:px-5">
          ...
          <div className="flex items-start gap-3 pr-10">
            <span
              className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-sky-100 text-sky-700 dark:bg-sky-950/50 dark:text-sky-300"
              aria-hidden
            >
            ...
            <div className="min-w-0">
              <h2
                id="previous-class-records-title"
                className="text-base font-semibold text-slate-900 dark:text-white"
              >
                Previous class records
              </h2>
              <p
                id="previous-class-records-desc"
                className="mt-1 text-sm text-slate-500 dark:text-zinc-400"
              >
                Subject records from before {studentName} joined this class.
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="min-h-0 flex-1 overflow-y-auto overflow-x-hidden px-4 py-4 sm:px-5">
          {loading ? (
            <div className="flex items-center justify-center gap-2 py-16 text-sm text-slate-500 dark:text-zinc-400">
              ...
              Loading previous class records…
            </div>
          ) : error ? (
            <p
              ...
              role="alert"
            >
              Could not load previous class records ({error}).
            </p>
          ) : groups.length === 0 ? (
            <HistoricalClassRecordsEmptyState />
          ) : (
            <HistoricalClassRecordsGroupedList groups={groups} />
          )}
        </div>
        ...
        <footer className="shrink-0 border-t border-slate-200 bg-slate-50/80 px-4 py-3 dark:border-zinc-700 dark:bg-zinc-800/50 sm:px-5">
          <p className="text-center text-xs leading-relaxed text-slate-500 dark:text-zinc-400">
            Historical class records are read-only and remain attached to the
            class where they were originally recorded.
          </p>
        </footer>
      </div>
    </div>,
