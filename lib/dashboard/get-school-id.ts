 ...
 *
 * Super admins only receive a school id when they explicitly opened a school
 * workspace (cookie) — never from membership fallbacks.
 */
export async function getSchoolIdForUser(
  ...
  if (await checkIsSuperAdmin(supabase, userId)) {
    return await readSuperAdminWorkspaceSchoolId();
  }
